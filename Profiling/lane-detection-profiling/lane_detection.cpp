...
long double laneDetection::lane_detection_pipeline(Mat src_in){
	...
	if (!lanes[0].size()){ return -1000.0; } 
	if (!lanes[1].size()){ return -1000.0; } 
	// cout << "here 7" << endl;
    ...
    imwrite("out_imgs/img_roi.png", img_roi);
    imwrite("out_imgs/img_diff_src_rebev.png", diff_src_rebev);
    imwrite("out_imgs/img_output.png", cropped); 
    imwrite("out_imgs/img_draw_lines.png", draw_lines); 
    imwrite("out_imgs/img_rev_warped.png", img_rev_warped); 
    imwrite("out_imgs/img_warped.png", img_warped);
    imwrite("out_imgs/img_gray.png", img_gray);
    if (maxVal){
        imwrite("out_imgs/img_white_mask.png", img_white_mask);
    } else {
        imwrite("out_imgs/img_thresholding.png", img_thresholding);
    }
    imwrite("out_imgs/img_lanes.png", img_lanes_temp);
...
vector<vector<Point>> laneDetection::sliding_window_lane_tracking(Mat& src){
    ...
    // Set the width of the windows to +/- margin
    int margin   = 65;
    // Set minimum number of pixels needed to recenter window
