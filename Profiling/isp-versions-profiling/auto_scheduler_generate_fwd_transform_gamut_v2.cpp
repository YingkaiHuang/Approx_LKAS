...
public:
    ...
    void generate() {      
        ...
        processed(x, y, c) = cast<uint8_t>( min( max( rbf_biases(x,y,c) * 255, 0), 255 ) );   
    ...
    void schedule() {
        if(!auto_schedule){
            ...
            Func processed = pipeline.get_func(9);
...
HALIDE_REGISTER_GENERATOR(AutoScheduled, auto_schedule_gen_fwd_v2)
