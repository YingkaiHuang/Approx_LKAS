...
// private methods
//--- convert image_2_Mat ---//
Mat vrepAPI::vrep_img_2_Mat(int the_iter){
	// 3 color image (RGB)
	const int b = 3; 
...
vrepAPI::vrepAPI() {
	// ------------ SIM_INIT start ---------------// 
	// connect to vrep, use sunchronous mode and start simulation
    simxFinish(-1);  
    m_clientID = simxStart((simxChar*)"127.0.0.1", 19997, true, true, 5000, 5);
    assert(("V-REP must be started", m_clientID != -1));
...
Mat vrepAPI::sim_sense(int the_iter) {
	// ------------ SENSE  ---------------// 
	...
	img_vrep = vrep_img_2_Mat(the_iter);
	return img_vrep;
}
